...
export default function TagList() {
  ...
  const fetchTagsData = clientApi.tag.list.useQuery()

  const isLoading = fetchTagsData.isLoading

  useEffect(() => {
    if (fetchTagsData.isSuccess && fetchTagsData.data.length > 0) {
      const records = fetchTagsData.data.map((record) => {
        return {
          ...record,
      ...
      setTags(records)
      setTotal(fetchTagsData.data.length)
      setTotalPages(Math.ceil(fetchTagsData.data.length / pageSize))
    } else {
      setTags([])
    }
  }, [fetchTagsData.isSuccess, fetchTagsData.data])
