/*
  ...
  - Added the required column `paymentAccount` to the `User` table without a default value. This is not possible if the table is not empty.
  - Added the required column `paymentName` to the `User` table without a default value. This is not possible if the table is not empty.
...
ADD COLUMN     "paymentAccount" TEXT NOT NULL,
...
CREATE TABLE "OrderMemo" (
    ...
    CONSTRAINT "OrderMemo_pkey" PRIMARY KEY ("id")
);
...
-- AddForeignKey
ALTER TABLE "OrderMemo" ADD CONSTRAINT "OrderMemo_orderId_fkey" FOREIGN KEY ("orderId") REFERENCES "Order"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
